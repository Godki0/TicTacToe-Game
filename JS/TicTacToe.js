...
function watch() {
    ...
    btnDisabled(btn); // disable the stop button sincethe game has not started
}
...
function rollForTurn() {
    ...
    // build the string to show which player rolledwhat die roll
    for (i=0;i<xArray.length;i++) {
        ...
        setTimeout(function() {writeMsg(txt1);}, 1000); // time delay for dramactic affect
    }
...
function startGame() {
    ...
    btnDisabled(btn); // disable the start button since the game is now running
    var btn = document.getElementById('btnStop');
    stopEnabled(btn); // enable the stop button since the game is now running
...
// this function is for the player configurationpanel and checks the
...
function saveSettings() {
    ...
    if (p1Selected[p1Index].text === p2Selected[p2Index].text) {
        alert("Error - Player 1 and Player 2 connot both be assigned as: "+p1Selected[p1Index].text)
    } else {
...
function avatarPlaced() {
    ...
    if (activePlayer == "Player 1") { // once active player selects a square change active player
...
function checkForWinCon() {
    ...
    info = info.split(','); // seperate the string bycommas into an array
...
function glowBoard(pos) {
    ...
    for (var i=0;i<squares.length;i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg3.style.backgroundColor = 'rgb(122, 244, 66)';}, 1000);
...
//-------------------------------------------------------------------
// checking for wincomn square 012
function checkForWinCon1(info,squareArray) {
    ...
    // this will triger (ONLY) if there was a match for index0, index1 and index2
...
// checking for woncon squares 345
...
// checking for woncon squares 678
function checkForWinCon3(info,squareArray) {
    ...
    if (match6Avatar != undefined && match7Avatar != undefined && match8Avatar != undefined) { // this will trigger (ONLY) if there was a match for index6, index7 and index8
...
// checking for woncon squares 036
function checkForWinCon4(info,squareArray) {
    var winCon4 = [0,3,6];
    var winDetected = "on";
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
    ...
    if (match0Avatar != undefined && match3Avatar != undefined && match6Avatar != undefined) { // this will trigger (ONLY) if there was a match for index0, index3 and index6
...
// checking for woncon squares 147
function checkForWinCon5(info,squareArray) {
    var winCon5 = [1,4,7];
    var winDetected = "on";
    for (var i in info) {
        if (info[i].charAt(0) == "1") {
    ...
    if (match1Avatar != undefined && match4Avatar != undefined && match7Avatar != undefined) { // this will trigger (ONLY) if there was a match for index1, index4 and index7
...
// checking for woncon squares 258
function checkForWinCon6(info,squareArray) {
    var winCon6 = [2,5,8];
    var winDetected = "on";
    for (var i in info) {
        if (info[i].charAt(0) == "2") {
    ...
    if (match2Avatar != undefined && match5Avatar != undefined && match8Avatar != undefined) { // this will trigger (ONLY) if there was a match for index2, index5 and index8
...
// checking for woncon squares 048
function checkForWinCon7(info,squareArray) {
    var winCon7 = [0,4,8];
    var winDetected = "on";
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
    ...
    if (match0Avatar != undefined && match4Avatar != undefined && match8Avatar != undefined) { // this will trigger (ONLY) if there was a match for index0, index4 and index8
...
// checking for woncon squares 246
function checkForWinCon8(info,squareArray) {
    var winCon8 = [2,4,6];
    var winDetected = "on";
    for (var i in info) {
        if (info[i].charAt(0) == "2") {
    ...
    if (match2Avatar != undefined && match4Avatar != undefined && match6Avatar != undefined) { // this will trigger (ONLY) if there was a match for index2, index4 and index6
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied
            var paintAvatar = determineAvatar(); // get the correctavatar to paint for the active player
            ...
            checkForWinCon(); // call function check if current move completes a winning condition
